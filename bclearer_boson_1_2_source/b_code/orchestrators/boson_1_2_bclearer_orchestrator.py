...
def orchestrate_boson_1_2_bclearer():
    ...
    gml_data_folder = select_folder(title="Select GML folder")

    set_up_logger_and_output_folder(output_folder_name=output_root_folder_path)
...
def __get_output_root_folder_path() -> str:
    ...
    return output_root_folder_path


def __process_gml_folder(output_folder_name: str, gml_data_folder_name: str) -> None:
    with EaToolsSessionManagers() as ea_tools_session_manager:

        evolved_universe = orchestrate_boson_1_2_bclearer_stages(
        ...
        __export_to_xml(
            evolved_universe=evolved_universe, output_folder_name=output_folder_name
        )


def __export_to_xml(evolved_universe, output_folder_name) -> None:
    ...
    xml_file_path = os.path.join(output_folder_name, "bOSON_1_2.xml")

    bnop_facade.write_bnop_object_to_xml(xml_file_path=xml_file_path)

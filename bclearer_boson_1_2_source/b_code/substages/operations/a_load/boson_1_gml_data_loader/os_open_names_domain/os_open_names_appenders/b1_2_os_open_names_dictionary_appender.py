...
def append_os_open_names_dictionary(
    os_open_names_dictionary: dict, file: Files
) -> dict:
    log_message(message="loading " + file.absolute_path_string)

    data = untangle.parse(file.absolute_path_string)

    log_message(message="parsing " + file.absolute_path_string)

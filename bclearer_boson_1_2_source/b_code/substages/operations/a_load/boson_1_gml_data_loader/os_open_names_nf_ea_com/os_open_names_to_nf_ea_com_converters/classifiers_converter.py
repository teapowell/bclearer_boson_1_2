...
from bclearer_boson_1_2_source.b_code.substages.operations.a_load.boson_1_gml_data_loader.os_open_names_nf_ea_com.os_open_names_to_nf_ea_com_converters.dependency_connectors_converter import (
...
def convert_to_classifiers(
    ...
    new_ea_classifiers = __create_new_ea_classifiers(
        os_open_names=os_open_names, root_package_nf_uuid=root_package_nf_uuid
    )
    ...
    nf_ea_com_universe = convert_dependency_connectors(
    ...
    __add_stereotype_usages(
        nf_ea_com_universe=nf_ea_com_universe,
        new_ea_classifiers=new_ea_classifiers,
        matched_stereotype=DigitalisationLevelStereotypeMatchedEaObjects.DIGITALISATION_LEVEL_5_CLASS_STEREOTYPE,
    )
...
def __create_new_ea_classifiers(
    os_open_names: DataFrame, root_package_nf_uuid: str
) -> DataFrame:
    ...
    new_ea_classifiers = dataframe_filter_and_rename(
    ...
    return new_ea_classifiers


def __add_stereotype_usages(
    nf_ea_com_universe: NfEaComUniverses,
    new_ea_classifiers: DataFrame,
    matched_stereotype: MatchedEaObjects,
) -> None:
    classifier_nf_uuids = set(new_ea_classifiers[NfColumnTypes.NF_UUIDS.column_name])

    new_ea_objects_dictionary = create_new_ea_objects_dictionary()

    stereotype_nf_uuid = get_nf_uuid_from_ea_guid_from_collection(
        nf_ea_com_universe=nf_ea_com_universe,
        collection_type=NfEaComCollectionTypes.EA_STEREOTYPES,
        ea_guid=matched_stereotype.ea_guid,
    )

    for classifier_nf_uuid in classifier_nf_uuids:
        add_new_stereotype_usage_to_dictionary(
            new_stereotype_usage_dictionary=new_ea_objects_dictionary[
                NfEaComCollectionTypes.STEREOTYPE_USAGE
            ],
            client_nf_uuid=classifier_nf_uuid,
            client_collection_type=NfEaComCollectionTypes.EA_CLASSIFIERS,
            stereotype_nf_uuid=stereotype_nf_uuid,
        )

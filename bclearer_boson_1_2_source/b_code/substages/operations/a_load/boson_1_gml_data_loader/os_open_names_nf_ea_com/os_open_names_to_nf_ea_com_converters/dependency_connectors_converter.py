...
def convert_dependency_connectors(
    ...
    log_message(message="adding dependencies to EA Connectors")
...
def __create_new_ea_connectors(
    ...
    classifier_type_nf_uuid = __get_classifier_type_nf_uuid(
        nf_ea_com_universe=nf_ea_com_universe,
        matched_type=matched_type,
        matched_association=matched_association,
    )
    ...
    new_ea_connectors[NfColumnTypes.NF_UUIDS.column_name] = new_ea_connectors.apply(
        lambda row: create_new_uuid(), axis=1
    )
...
def __get_classifier_type_nf_uuid(
    ...
    if matched_association is None:
        return matched_type_nf_uuid

    else:
        linked_by_association_nf_uuid = get_nf_uuid_from_ea_guid_from_collection(
            nf_ea_com_universe=nf_ea_com_universe,
            collection_type=NfEaComCollectionTypes.EA_CLASSIFIERS,
            ea_guid=matched_association.ea_guid,
        )

        return get_intersection_of_dependency_and_association_linked(
            nf_ea_com_universe=nf_ea_com_universe,

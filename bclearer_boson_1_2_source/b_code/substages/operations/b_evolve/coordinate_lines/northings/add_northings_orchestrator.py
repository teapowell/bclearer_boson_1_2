...
from bclearer_boson_1_2_source.b_code.common_knowledge.coordinate_line_dimensions import (
    CoordinateLineDimensions,
)
...
def orchestrate_add_northings(
    ...
    northings_exemplars = get_coordinate_line_exemplars_from_xml_test_attributes(
        nf_ea_com_universe=content_universe,
        dimension=CoordinateLineDimensions.NORTHINGS,
    )

    new_ea_objects_dictionary = create_new_ea_objects_dictionary()

    package_nf_uuid = create_root_package(
        nf_ea_com_universe=output_universe, package_name="Coordinate Lines - Northings"
    )

    type_nf_uuid = get_nf_uuid_from_ea_guid_from_collection(
        nf_ea_com_universe=output_universe,
        collection_type=NfEaComCollectionTypes.EA_CLASSIFIERS,
        ea_guid=Boson12MatchedEaObjects.EPSG_27700_NORTHING_COORDINATE_LINES.ea_guid,
    )

    name_nf_uuid = get_nf_uuid_from_ea_guid_from_collection(
        nf_ea_com_universe=output_universe,
        collection_type=NfEaComCollectionTypes.EA_CLASSIFIERS,
        ea_guid=Boson12MatchedEaObjects.EPSG_27700_NORTHING_COORDINATE_LINE_NAMES.ea_guid,
    )

    character_string_nf_uuid = get_nf_uuid_from_ea_guid_from_collection(
        nf_ea_com_universe=output_universe,
        collection_type=NfEaComCollectionTypes.EA_CLASSIFIERS,
        ea_guid=BclearerMatchedEaObjects.CHARACTER_STRINGS.ea_guid,
    )

    named_by_stereotype_nf_uuid = get_nf_uuid_from_ea_guid_from_collection(
        nf_ea_com_universe=output_universe,
        collection_type=NfEaComCollectionTypes.EA_STEREOTYPES,
        ea_guid=BclearerMatchedEaObjects.NAMED_BY_STEREOTYPE.ea_guid,
    )

    line_digitalisation_level_stereotype_nf_uuid = get_nf_uuid_from_ea_guid_from_collection(
        nf_ea_com_universe=output_universe,
        collection_type=NfEaComCollectionTypes.EA_STEREOTYPES,
        ea_guid=DigitalisationLevelStereotypeMatchedEaObjects.DIGITALISATION_LEVEL_4_CLASS_STEREOTYPE.ea_guid,
    )

    name_digitalisation_level_stereotype_nf_uuid = get_nf_uuid_from_ea_guid_from_collection(
        nf_ea_com_universe=output_universe,
        collection_type=NfEaComCollectionTypes.EA_STEREOTYPES,
        ea_guid=DigitalisationLevelStereotypeMatchedEaObjects.DIGITALISATION_LEVEL_1_CLASS_STEREOTYPE.ea_guid,
    )

    for northings_exemplar in northings_exemplars:
        new_ea_objects_dictionary = add_coordinate_line_structure_to_dictionary(
            new_ea_objects_dictionary=new_ea_objects_dictionary,
            exemplar=northings_exemplar,
            suffix="Northing",

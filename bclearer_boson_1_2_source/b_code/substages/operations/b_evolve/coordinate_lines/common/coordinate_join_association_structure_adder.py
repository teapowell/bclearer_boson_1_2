...
def add_coordinate_join_association_structure(
    ...
    eastings_dictionary = get_nf_uuid_keyed_on_name_prefix_dictionary(
        nf_ea_com_universe=nf_ea_com_universe,
        matched_type=easting_type_matched_ea_object,
        prefix_word_count=1,
    )

    northings_dictionary = get_nf_uuid_keyed_on_name_prefix_dictionary(
        nf_ea_com_universe=nf_ea_com_universe,
        matched_type=northing_type_matched_ea_object,
        prefix_word_count=1,
    )

    points_dictionary = get_nf_uuid_keyed_on_name_prefix_dictionary(
        nf_ea_com_universe=nf_ea_com_universe,
        matched_type=point_type_matched_ea_object,
        prefix_word_count=2,
    )

    join_association_type_nf_uuid = get_nf_uuid_from_ea_guid_from_collection(
        nf_ea_com_universe=nf_ea_com_universe,
...
def __add_coordinate_join_association(
    ...
    easting_and_northing = point_exemplar.split(" ")

    easting = easting_and_northing[0]

    northing = easting_and_northing[1]

    easting_nf_uuid = eastings_dictionary[easting]

    northing_nf_uuid = northings_dictionary[northing]

    association_nf_uuid = add_new_association_class_to_dictionary(

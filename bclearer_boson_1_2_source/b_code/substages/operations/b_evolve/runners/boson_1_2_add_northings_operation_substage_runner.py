...
from bclearer_boson_1_2_source.b_code.substages.operations.b_evolve.coordinate_lines.northings.add_northings_orchestrator import (
    orchestrate_add_northings,
)


def run_boson_1_2_add_northings_operation_substage(
    content_universe: NfEaComUniverses,
    ea_tools_session_manager: EaToolsSessionManagers,
    bespoke_operation_configuration: BespokeOperationConfigurations,
) -> NfEaComUniverses:
    log_message(
        message="CONTENT OPERATION: Add northings to universe - "
        + bespoke_operation_configuration.short_name
        + " - started"
    )

    output_universe = orchestrate_add_northings(

...
from bclearer_source.b_code.substages.operations.common.class_adder import (
    add_new_class_to_dictionary,
)
from bclearer_source.b_code.substages.operations.common.connector_adder import (
...
from bclearer_source.b_code.substages.operations.common.stereotype_adder import (
...
from bclearer_boson_1_2_source.b_code.common_knowledge.boson_1_2_matched_ea_objects import (
    Boson12MatchedEaObjects,
)
from bclearer_boson_1_2_source.b_code.common_knowledge.inspire_matched_ea_objects import (
...
def orchestrate_deduplicate_points(
    ...
    output_universe = create_empty_nf_ea_universe(
        ea_tools_session_manager=ea_tools_session_manager, short_name=short_name
    )
    ...
    original_gm_points = get_instances_nf_uuids_of_matched_type(
    ...
    remove_original_gm_points(
        output_universe=output_universe, original_gm_points=original_gm_points
    )

    remove_connector_by_name(
        nf_ea_com_universe=output_universe, connector_name_to_delete="lowerCorner"
    )

    remove_connector_by_name(
        nf_ea_com_universe=output_universe, connector_name_to_delete="upperCorner"
    )
...
def __add_new_objects(
    output_universe: NfEaComUniverses, new_ea_objects_dictionary: dict
) -> None:
    package_nf_uuid = create_root_package(
        nf_ea_com_universe=output_universe,
        package_name="Coordinate Points - Unique Points",
    )

    point_boson_type_nf_uuid = get_nf_uuid_from_ea_guid_from_collection(
        nf_ea_com_universe=output_universe,
    ...
    point_inspire_type_nf_uuid = get_nf_uuid_from_ea_guid_from_collection(
        nf_ea_com_universe=output_universe,
        collection_type=NfEaComCollectionTypes.EA_CLASSIFIERS,
        ea_guid=InspireMatchedEaObjects.GM_POINT.ea_guid,
    )

    point_exemplars = get_coordinate_point_exemplars_from_xml_test_attributes(
        nf_ea_com_universe=output_universe
    )

    geometry_associated_nf_uuids = (
        get_nf_uuid_sets_by_association_keyed_on_name_prefix_dictionary(
            nf_ea_com_universe=output_universe, association_name="geometry"
        )
    )

    lower_corner_associated_nf_uuids = (
        get_nf_uuid_sets_by_association_keyed_on_name_prefix_dictionary(
            nf_ea_com_universe=output_universe, association_name="lowerCorner"
        )
    )

    upper_corner_associated_nf_uuids = (
        get_nf_uuid_sets_by_association_keyed_on_name_prefix_dictionary(
            nf_ea_com_universe=output_universe, association_name="upperCorner"
        )
    )

    point_name_instances_dictionary = get_nf_uuid_sets_keyed_on_name_prefix_dictionary(
        nf_ea_com_universe=output_universe,
...
def __add_point_to_dictionary(
    ...
    point_name_nf_uuid = add_new_class_to_dictionary(
    ...
    if point_exemplar in lower_corner_associated_nf_uuids:
        for associated_lower_corner_nf_uuid in lower_corner_associated_nf_uuids[
            point_exemplar
        ]:
            add_new_connector_to_dictionary(
                new_connector_dictionary=new_ea_objects_dictionary[
    ...
    if point_exemplar in upper_corner_associated_nf_uuids:
        for associated_upper_corner_nf_uuid in upper_corner_associated_nf_uuids[
            point_exemplar
        ]:
            add_new_connector_to_dictionary(
                new_connector_dictionary=new_ea_objects_dictionary[
    ...
    if point_exemplar in point_name_instances_dictionary:
        for point_name_instance_nf_uuid in point_name_instances_dictionary[
            point_exemplar
        ]:
            add_new_connector_to_dictionary(
                new_connector_dictionary=new_ea_objects_dictionary[

...
def get_nf_uuid_sets_by_association_keyed_on_name_prefix_dictionary(
    nf_ea_com_universe: NfEaComUniverses, association_name: str
) -> dict:
    exemplar_and_nf_uuids_columns = __get_names_and_nf_uuid_sets_by_association(
        nf_ea_com_universe=nf_ea_com_universe, association_name=association_name
    )

    nf_uuids_indexed_on_exemplars_dictionary = __convert_dataframe_to_dictionary(
        exemplar_and_nf_uuids_columns=exemplar_and_nf_uuids_columns
    )

    return nf_uuids_indexed_on_exemplars_dictionary
...
def __get_names_and_nf_uuid_sets_by_association(
    ...
    filtered_associations = ea_associations[
        ea_associations[NfEaComColumnTypes.EXPLICIT_OBJECTS_EA_OBJECT_NAME.column_name]
        == association_name
    ]
    ...
    name_column_name = NfEaComColumnTypes.EXPLICIT_OBJECTS_EA_OBJECT_NAME.column_name

    instances_of_coordinate_point_type = left_merge_dataframes(
    ...
    prefix_and_nf_uuids_columns = instances_of_coordinate_point_type[
        [
            "Prefixes",
    ...
    return prefix_and_nf_uuids_columns


def __convert_dataframe_to_dictionary(exemplar_and_nf_uuids_columns: DataFrame) -> dict:
    nf_uuids_grouped_by_names = exemplar_and_nf_uuids_columns.groupby("Prefixes")[
        NfEaComColumnTypes.ELEMENTS_SUPPLIER_PLACE1_END_CONNECTORS.column_name
    ].apply(set)

    nf_uuid_sets_keyed_on_names = nf_uuids_grouped_by_names.to_dict()

    return nf_uuid_sets_keyed_on_names

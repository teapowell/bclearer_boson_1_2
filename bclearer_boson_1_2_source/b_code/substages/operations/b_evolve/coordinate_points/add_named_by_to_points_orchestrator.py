...
def orchestrate_add_named_by_to_points(
    ...
    points_keyed_on_exemplars = get_nf_uuid_keyed_on_name_prefix_dictionary(
        nf_ea_com_universe=output_universe,
        matched_type=Boson12MatchedEaObjects.EPSG_27700_COORDINATE_POINTS,
        prefix_word_count=2,
    )

    point_names_keyed_on_exemplars = get_nf_uuid_keyed_on_name_prefix_dictionary(
        nf_ea_com_universe=output_universe,
...
def __add_named_by(
    ...
    point_name_nf_uuid = point_names_keyed_on_exemplars[point_exemplar]

    named_by_association_nf_uuid = add_new_connector_to_dictionary(
